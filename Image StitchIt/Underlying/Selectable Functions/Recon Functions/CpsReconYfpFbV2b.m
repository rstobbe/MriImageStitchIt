...
classdef CpsReconYfpFbV2b < handle
...
function InitViaCompass(obj,ReconIpt)    
    ...
    if strcmp(ReconIpt.('DataTests'),'Yes')
        obj.Recon.SetTestData(1);
    end 
    if strcmp(ReconIpt.('DisplayRxProfs'),'Yes')
        obj.Recon.SetDisplayRxProfs(1);
    end 
    FlipStr = ReconIpt.('NavigatorFlip');
    for n = 1:length(FlipStr)
        Flip(n) = str2double(FlipStr(n));
    end
    obj.Recon.SetNavigatorFlip(Flip);
...
function [Interface] = CompassInterface(obj,SCRPTPATHS)    
    ...
    m = m+1;
    Interface{m,1}.entrytype = 'Input';
    Interface{m,1}.labelstr = 'NavigatorFlip';
    Interface{m,1}.entrystr = '0';
    m = m+1;
    Interface{m,1}.entrytype = 'Choose';
    Interface{m,1}.labelstr = 'DataTests';
    Interface{m,1}.entrystr = 'No';
    Interface{m,1}.options = {'Yes','No'};
    m = m+1;
    Interface{m,1}.entrytype = 'Choose';
